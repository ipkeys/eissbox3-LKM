...
MODULE_AUTHOR("Yuriy Kozhynov");  // The author visible with modinfo
MODULE_DESCRIPTION("A simple Linux LKM for the Ebox3."); // desc.
...
static int __init helloEbox3_init(void) {
   printk(KERN_INFO "Ebox3: Hello %s from the Ebox3 LKM!\n", name);
   return 0;
}
...
static void __exit helloEbox3_exit(void) {
   printk(KERN_INFO "Ebox3: Goodbye %s from the Ebox3 LKM!\n", name);
}
...
module_init(helloEbox3_init);
module_exit(helloEbox3_exit);
